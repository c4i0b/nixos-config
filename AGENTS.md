...
# can't read the machine-local /etc/nixos/hardware-configuration.nix.
```
`flake.lock` is committed and reviewed like any other change.

## Structure
- `/etc/nixos` — machine-local `hardware-configuration.nix` (generated by `nixos-generate-config`; imported per host, gitignored)
- `flake.nix` / `flake.lock` — entry point; pins nixpkgs + home-manager flake inputs
- `hosts/nixos/` — host composition (`default.nix`); imports `/etc/nixos/hardware-configuration.nix`
...
- 13 State & Maintenance → `modules/nixos/base.nix` (gc keeps 7d, optimise; autoUpgrade pulls main from GitHub)
