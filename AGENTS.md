# NixOS Configuration for c4i0b/nixos-config

## Environment

Nix installed as single-user at `~/.nix-profile/`. Every `nix` or `nixos-*` command requires prepending:
```bash
export PATH="$HOME/.nix-profile/bin:$PATH"
export NIX_CONFIG="experimental-features = nix-command flakes"
```

Shell is `fish` — the nix profile script has a bash syntax error, source `.nix-profile/etc/profile.d/nix.sh` instead.

## Commands

```bash
nix flake check                              # validate config (files must be git-tracked first)
nix fmt                                       # format with alejandra (via flake formatter)
git -C /etc/nixos add -A && sudo nixos-rebuild switch --flake /etc/nixos
```

For privilege escalation (no TTY): `/run/wrappers/bin/pkexec env PATH="$HOME/.nix-profile/bin:/run/current-system/sw/bin:$PATH" nixos-rebuild switch --flake /etc/nixos`

## Architecture

Template base: `standard/` from Misterio77/nix-starter-configs.

```
flake.nix                          inputs, overlays, nixosConfigurations
hosts/desktop/                      host-specific modules (hardware, GPU, peripherals)
hosts/desktop/default.nix          host-specific config (mounts, wireplumber rules)
hosts/desktop/hardware-configuration.nix  generated by nixos-generate-config
hosts/desktop/gpu.nix              NVIDIA config
nixos/configuration.nix            orchestrator: imports shared modules + nix settings
nixos/*.nix                        per-domain modules (one concern per file, shared across hosts)
overlays/default.nix               additions + modifications + unstable-packages
pkgs/default.nix                   custom derivations
modules/nixos/                     reusable NixOS modules (exported via flake)
modules/home-manager/              reusable home-manager modules (exported via flake)
```

- **Shared modules** live in `nixos/` — imported by `configuration.nix`, available to all hosts
- **Host-specific modules** live in `hosts/<hostname>/` — imported in `flake.nix` per configuration
- New domain = new file in `nixos/` + add to `imports` in `configuration.nix`
- New host = new dir in `hosts/` + new `nixosConfiguration` entry in `flake.nix`
- Config stays **inline in `nixos/*.nix`** — no split sub-modules
- `hardware-configuration.nix` is generated by `nixos-generate-config`, not edited manually
- Dotfiles managed via GNU stow, not home-manager

### Overlay system

- `additions`: custom packages from `pkgs/` + packages from external flakes
- `modifications`: package overrides and patches
- `unstable-packages`: exposes `pkgs.unstablePkgs` from nixpkgs-unstable; use for actively-developed tools that need latest releases

## Patterns

### When in doubt, search first
- Before writing custom config, search the web (NixOS wiki, nixpkgs manual, GitHub, discourse) for existing patterns
- Before adding a new service or feature, check if nixpkgs already has a module for it — many things are a one-liner
- Prefer upstream module options over manual config — they handle edge cases and survive refactors
- Use `nix-search <query>` to find packages; `nix-search --exact false <query>` for fuzzy/broader matches
- Run `nix-search --index --flake nixpkgs` periodically to keep the index fresh

### Maintainability
- When integrating modules, prefer the approach that minimizes future maintenance burden: less manual config, more declarative options
- Avoid tinkering with internal module workings — use the public API (module options), not by overriding internal derivations
- If a module option exists, use it; only drop to raw config files as last resort

### Adding a new system package
1. Open `nixos/packages.nix`
2. Add to `environment.systemPackages`
3. Prefer stable nixpkgs; use `unstablePkgs.xxx` only for actively-developed tools

### Adding a new domain module
1. Create `nixos/<domain>.nix` with standard header `{pkgs, ...}: { ... }`
2. Add `./<domain>.nix` to `imports` in `configuration.nix`
3. If it depends on external flakes, add the input in `flake.nix` and pass via `specialArgs`

### Switching desktop environment
- Comment out the current DE module import in `configuration.nix`, add the new one — never delete, enables easy revert
- GPU module (`gpu.nix`) is DE-agnostic, keep as-is

### Rebuild workflow
1. `git add` new files (nix refuses to evaluate untracked files)
2. Run `nix flake check` to validate
3. Run the rebuild command above

### Push after meaningful changes
- After rebuild succeeds, push to remote: `git push origin main`
- Push early, push often — remote is backup and enables `nixos-rebuild` from flake URL on other machines

## Key constraints

- Keep template comment markers: `# Opinionated:`, `# FIXME`, `# TODO`, `# > Our main nixos configuration file <`
